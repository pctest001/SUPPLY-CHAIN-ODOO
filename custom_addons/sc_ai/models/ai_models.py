...
PROMPT_REGISTRY = {
    'v1': ('你是供应链智能助手，服务于基于 Odoo 的流程制造供应链系统。'
           ...
           '所有数据查询均继承用户权限，禁止越权。'),
    # v2（2026-07-27）：生产回流 PROD-61（注入未拒真实 case）+ L4 live 评测
    # E07/E08/E09/E12/E14 暴露的漏拒/复述敏感词问题 → 安全规则强化
    ...
    # v3（2026-07-28）：生产回流 PROD-H2/H3/H5/H6（数量类幻觉真实 case）— 工具正常返回后
    # AI 仍自行拆分/估算数量（如"16 正常商品/34 系统物料""共 51 项""17 种""16 项"），
    # 这些数字工具均未返回 → quantity_grounding 判幻觉。v2 第4条只防"工具报错时编造"，
    # 未防"工具正常返回后 AI 自行加工数量"，v3 补强：汇报数量必须以工具返回行数为准。
